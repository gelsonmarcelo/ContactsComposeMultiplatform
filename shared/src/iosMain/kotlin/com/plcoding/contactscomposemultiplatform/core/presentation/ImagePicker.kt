    ...
    private val delegate = object : NSObject(), UIImagePickerControllerDelegateProtocol,
        ...
        override fun imagePickerController(
            ...
            val imageNsData = UIImageJPEGRepresentation(didFinishPickingImage, 1.0)
                ?: return
            val bytes = ByteArray(imageNsData.length.toInt()) //Todo declaring the array of bytes with the size of the image data
            memcpy(bytes.refTo(0), imageNsData.bytes, imageNsData.length) //Todo copy the image data to bytes array

            onImagePicked(bytes)

            picker.dismissViewControllerAnimated(true, null) //Todo Close the view
        }
